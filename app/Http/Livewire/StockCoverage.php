...
class StockCoverage extends Component
{
...
//                            'BasePrice' => $branchRows->sum('BasePrice'),
                            ...
                            'InventoryValue' => $branchRows->sum('InventoryValue'),
                            'AnnualQuantitySale' => $branchRows->sum('AnnualQuantitySale'),
                            // better than sum because sufficiency is a ratio
                            'SufficiencyDays' => $branchRows->sum('AnnualQuantitySale') > 0
                                ? round(($branchRows->sum('InventoryQuantity') / $branchRows->sum('AnnualQuantitySale')) * 365, 2)
                                : null,
//                            'SufficiencyDays' => $branchRows->sum('SufficiencyDays')
...
//                            'employees' => $employees,
                    ...
                    ->filter(function ($branch) {
                        ...
                        return $branch['SufficiencyDays'] !== null
                            && $branch['SufficiencyDays'] >= (float) $this->sufficiency_days;
                    })
...
//            dd($this->itemGrp);
    ...
    {
        return view('livewire.inventory.stock-coverage')->layout('layouts.dashboard');
    }
...
//    public function create_report(){
    ...
    public function create_report($start_date, $end_date, $dept_id, $group_type, $cat_type, $sp_type, $vendor_type, $search_type, $product_code, $catalog_number, $marketing_type, $item_validity, $customer_type, $emps_type) {
        ...
        $this->sapQuery($start_date, $dept_id, $dept_id, $customer_type, $emps_type, $item_validity , );
...
//            dd($this->vendor_list);
    ...
    public function sapQuery( $item_validity, $departments)
    {
...
(
    ...
    (
        ...
        FROM AL_YASEEN_AGRI_PLIVE."OINV" H
        INNER JOIN AL_YASEEN_AGRI_PLIVE."INV1" L ON H."DocEntry" = L."DocEntry"
        WHERE H."CANCELED" = 'N'
          AND H."DocDate" BETWEEN ADD_DAYS(CURRENT_DATE, -365) AND CURRENT_DATE
        ...
        SELECT
            L."ItemCode",
            L."WhsCode",
            SUM(L."Quantity" * -1) AS "Qty"
        FROM AL_YASEEN_AGRI_PLIVE."ORIN" H
        INNER JOIN AL_YASEEN_AGRI_PLIVE."RIN1" L ON H."DocEntry" = L."DocEntry"
        WHERE H."CANCELED" = 'N'
          AND H."DocDate" BETWEEN ADD_DAYS(CURRENT_DATE, -365) AND CURRENT_DATE
        GROUP BY L."ItemCode", L."WhsCode"
    ) X
    GROUP BY X."ItemCode", X."WhsCode"
)

SELECT
    T0."CardCode" AS "VendorCode",
    BP."CardName" AS "VendorName",

    T0."ItemCode" AS "ItemCode",
    T0."ItemName" AS "ItemName",

   -- T0."QryGroup1"  AS "مميز0",
   -- T0."QryGroup2"  AS "مميز1",
  --  T0."QryGroup3"  AS "مميز2",
   -- T0."QryGroup30" AS "ادارة فنية-الاسمدة",
  --  T0."QryGroup31" AS "ادارة فنية-المبيدات",
   -- T0."QryGroup32" AS "ادارة فنية-البذور",
   -- T0."QryGroup40" AS "الحدائق الصحة العامة",
   -- T0."QryGroup41" AS "المكافحة المتكاملة",
  --  T0."QryGroup50" AS "الاليات",
  --  T0."QryGroup51" AS "الري",
  ---  T0."QryGroup52" AS "نترا",
  --  T0."QryGroup53" AS "الخدمات",
    ...
    T0."InvntryUom" AS "Unit",

    W."WhsCode" AS "InventoryCode",

    W."WhsName" AS "InventoryName",

    B."BPLId" AS "BPLId",
    B."BPLName" AS "BPLName",

    T2."OnHand" AS "InventoryQuantity",

    IFNULL(P1."Price", 0) AS "BasePrice",

    --T2."AvgPrice" AS "كلفة الوحدة",
    T2."AvgPrice" AS "Cost",

    --T2."OnHand" * T2."AvgPrice" AS "كلفة المخزون",
    T2."OnHand" * T2."AvgPrice" AS "InventoryCost",

   -- T2."OnHand" * IFNULL(P1."Price", 0) AS "قيمة المخزون",
    T2."OnHand" * IFNULL(P1."Price", 0) AS "InventoryValue",

  --  IFNULL(S."AnnualQty", 0) AS "كمية المبيعات السنوية",
    IFNULL(S."AnnualQty", 0) AS "AnnualQuantitySale",

    CASE
        WHEN IFNULL(S."AnnualQty", 0) <= 0 THEN NULL
        ELSE ROUND((T2."OnHand" / S."AnnualQty") * 365, 2)
   -- END AS "ايام الكفاية",
    END AS "SufficiencyDays",

    CURRENT_DATE AS "تاريخ التقرير"

FROM AL_YASEEN_AGRI_PLIVE."OITM" T0

INNER JOIN AL_YASEEN_AGRI_PLIVE."OITW" T2
    ON T0."ItemCode" = T2."ItemCode"

INNER JOIN AL_YASEEN_AGRI_PLIVE."OWHS" W
    ON T2."WhsCode" = W."WhsCode"

LEFT JOIN AL_YASEEN_AGRI_PLIVE."OBPL" B
    ON B."BPLId" = W."BPLid"

LEFT JOIN AL_YASEEN_AGRI_PLIVE."OCRD" BP
    ON BP."CardCode" = T0."CardCode"

LEFT JOIN AL_YASEEN_AGRI_PLIVE."ITM1" P1
    ON P1."ItemCode" = T0."ItemCode"
   AND P1."PriceList" = 1

LEFT JOIN SalesQty S
    ON S."ItemCode" = T0."ItemCode"
   AND S."WhsCode" = T2."WhsCode"

WHERE T0."InvntItem" = 'Y'
  AND T2."OnHand" <> 0
  AND T0."ItemCode" IN ({$selectedItemCodes})
SQL;
...
// -------- Vendor Filter (multi) --------
            ...
            if (!empty($vendor_type) && !in_array('vendor_all', $vendor_type, true)) {
                ...
                $sql .= $escaped = implode(',', $escaped);
                $sql .=<<<SQL
 AND I."CardCode" IN $escaped
 SQL;
            }
// -------- Speciality Filter (multi) --------
            if (!empty($sp_type) && !in_array('sp_all', $sp_type)) {
                ...
                if ($spConditions)
                    ...
                    $spConditions = implode(' OR ', $spConditions);
                $sql .= <<<SQL
 AND  $spConditions
 SQL;
            }
...
//
            ...
            if (!empty($marketing_type) && !in_array('marketing_all', $marketing_type, true)) {
                ...
                if (!empty($mrktConditions)) {
                    $mrktConditions = implode(' OR ', $mrktConditions);
                    $sql .= <<<SQL
 AND  $mrktConditions
 SQL;
                }
...
//
//            }
//            if (!empty($this->sufficiency_days)) {
//                $sql .= <<<SQL
//             AND "SufficiencyDays" >= $this->sufficiency_days
//     SQL;
//
//            }
            $sql .=<<<SQL
