...
class ListAgingByCustomerAndEmployee extends Component
{
    use WithPagination;
...
//        $this->emit('show-container');
        ...
        $this->last_date = date($this->selected_date);
...
ORDER BY T0."SlpCode", T0."CardCode"';
            ...
            else
            {
                while ($row = odbc_fetch_array($result)) {
                    ...
                    $this->customer_credit_limit[$row["CardCode"]] = $row["CreditLine"];
                    $this->customer_payment_term[$row["CardCode"]] = $row["PymntGroup"];
                    $this->valid_for[$row["CardCode"]] = $row["validFor"];
                }
