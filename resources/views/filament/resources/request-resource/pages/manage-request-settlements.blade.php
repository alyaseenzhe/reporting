    ...
    <div class="space-y-6">
            ...
            <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-600">
                            Request #{{ $requestRecord['id'] }} created by {{ $requestRecord['user_name'] ?? '-' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    ...
                    <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-center shadow-sm">
                        <div class="text-xs font-medium text-emerald-700">Done</div>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="px-6 py-4">
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-sm">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <div class="text-sm font-semibold text-slate-900">Upload attachment</div>
                            <p class="mt-1 text-sm text-slate-500">Choose a file and click the button to save it to media.</p>
                        </div>
                    ...
                    <form id="attachment-upload-form" wire:submit.prevent="uploadAttachment" class="mt-6 space-y-3">
                        <label class="block">
                            <span class="text-sm font-medium text-slate-700">Select file</span>
            ...
            @if (! empty($attachments))
                <div class="border-t border-slate-200 bg-slate-50 px-6 py-4">
                    <div class="text-sm font-semibold text-slate-700">Current attachments</div>
                    <div class="mt-3 space-y-3">
                        @foreach ($attachments as $attachment)
                            <div class="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3">
                                ...
                                <a href="{{ $attachment['url'] }}" target="_blank" class="text-primary-600 underline">Open</a>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        @if (empty($settlements))
            <div class="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
                <div class="text-lg font-semibold text-slate-800">No settlements are available for your permission scope.</div>
                <p class="mt-2 text-sm text-slate-500">This page uses the same department permission rules as the old settlement manager.</p>
            </div>
        @else
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 bg-slate-50 px-6 py-4">
                    <div class="grid grid-cols-[1fr_auto_auto] items-center gap-4 text-sm font-semibold text-slate-600">
                        <div>Settlement</div>
                        <div>Department</div>
                        <div>Done</div>
                    </div>
                ...
                <div class="divide-y divide-slate-100">
                    @foreach ($settlements as $settlement)
                        ...
                        <label wire:key="settlement-row-{{ $settlement['id'] }}" class="grid cursor-pointer grid-cols-[1fr_auto_auto] items-center gap-4 px-6 py-4 transition hover:bg-slate-50">
                            <div class="min-w-0">
                                ...
                                <div class="mt-1 text-sm {{ $isDone ? 'text-emerald-600' : 'text-slate-500' }}">
                                    {{ $isDone ? 'Completed' : 'Waiting to be completed' }}
                                </div>
                            </div>
