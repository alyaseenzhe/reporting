                        ...
                        <div>
                            @if($record["CardCode"] != $vendor_id)
                                ...
                                <tr style="background-color: #dcdcdc; border: 2px solid black; font-weight: bold">
                                    <td style="border: 2px solid black;background-color: #dcdcdc" class="border p-2 whitespace-nowrap col-id-no" scope="row">{{ $record["CardCode"] }}</td>
                                    <td colspan="29" style="border: 2px solid black;background-color: #dcdcdc" class="border p-2 whitespace-nowrap col-id-no" scope="row">{{ $record["DefaultPreferredVendor"] }}</td>
                                </tr>
                            @endif
                                <?php $vendor_id = $record["CardCode"]; ?>
                            <tr>
                                <th colspan="30" style="border: 2px solid black; background-color: #faebd7" class="col-id-no fixed-header border p-2 whitespace-nowrap">
                                    <div class="flex flex-row">
                                        ...
                                        <div style="color: #fd0e0e" class="w-full text-sm text-center">{{ $record["ItemCode"] }}</div>
                                        <div class="w-full text-sm text-center">اسم الصنف</div>
                                        <div style="color: #fd0e0e" class="w-full text-sm text-center">{{ $record["ItemDescription"] }}</div>
                                        <div class="w-full text-sm text-center">الوحدة</div>
                                        <div style="color: #fd0e0e" class="w-full text-sm text-center">{{ $record["SalUnitMsr"] }}</div>
                                        ...
                                        <div style="color: #fd0e0e" class="w-full text-sm text-center">{{ $record["DefaultPreferredVendor"] }}</div>
                                        ...
                                        <div style="color: #fd0e0e" class="w-full text-sm text-center">{{ number_format($record["ItemPrice"], 2) }}</div>
...
//                                                        $new_result = key_exists('ProductCode', $record) ? $results->where('ProductCode', $record['ProductCode'])->where('EmpCode', $emp->emp_code)->first(): 0;
                                            ...
                                            <th style="border: 2px solid black; z-index: 10; background-color: #e4fdf7;" class="border">
                                                    <?php
                                                        $product_id = \App\Models\Products::where('sap_code', $record["ItemCode"])->pluck('product_code')->first();
                                                    ...
                                                    //$new_result = \Illuminate\Support\Facades\DB::select("select target from `product_targets` where `product_id` = '". $record["ItemCode"] ."' AND `branch` = '". $dept_id[0] ."' AND `month` = '".$month."' AND `year` = '". intval($year_key) ."' and user_id ='". $emp->id ."'");
//                                                    $new_result = \Illuminate\Support\Facades\DB::select("select target from `product_targets` where `sap_code` = '". $record["ItemCode"] ."' AND `branch` = '". $old_dept_id[$dept_id[0]] ."' AND `month` = '".$month."' AND `year` = '". intval($year_key) ."' and user_id ='". $emp->id ."'")
                                                  //   $new_result = \Illuminate\Support\Facades\DB::select("select target from `product_targets` where `product_id` = '". $product_id ."' AND `branch` = '". $dept_id[0] ."' AND `month` = '".$month."' AND `year` = '". intval($year_key) ."' and user_id ='". $emp->id ."'");

//
                                                    $new_result = \Illuminate\Support\Facades\DB::select("
    SELECT target
    FROM product_targets
    WHERE
    (
        (sap_code IS NOT NULL AND sap_code = ?)
        OR
        (sap_code IS NULL AND product_id = ?)
    )
    AND branch = ?
    AND month = ?
    AND year = ?
...
", [
                                ...
                                @php
                                    ...
                                    $total_s_value = $total_s_value + $total_new_sales*$record["ItemPrice"];
                                    $total_f_value = $total_f_value + $total_new_tr*$record["ItemPrice"];

                                    if (!array_key_exists($emp->emp_code, $all_total_emp_sales)) {
                                            $all_total_emp_sales[$emp->emp_code] = $total_new_sales*$record["ItemPrice"];
                                        }
                                        else {
                                            $all_total_emp_sales[$emp->emp_code] += $total_new_sales*$record["ItemPrice"];
                                        }
                                        ...
                                        if (!array_key_exists($emp->emp_code, $all_total_emp_tr)) {
                                            $all_total_emp_tr[$emp->emp_code] = $total_new_tr*$record["ItemPrice"];
                                        }
                                        else {
                                            $all_total_emp_tr[$emp->emp_code] += $total_new_tr*$record["ItemPrice"];
                                        }
                    ...
                    @foreach($results as $record)
                        <div>
                            @if($record["CardCode"] != $vendor_id)
                                ...
                                <tr style="background-color: #dcdcdc; border: 2px solid black; font-weight: bold">
                                    <td style="border: 2px solid black;background-color: #dcdcdc" class="border p-2 whitespace-nowrap col-id-no" scope="row">{{ $record["CardCode"] }}</td>
                                    <td colspan="31" style="border: 2px solid black;background-color: #dcdcdc" class="border p-2 whitespace-nowrap col-id-no" scope="row">{{ $record["DefaultPreferredVendor"] }}</td>
                                </tr>
                            @endif
                                <?php $vendor_id = $record["CardCode"] ?>
                            <tr>
...
//                                                        $month_num = "month".$month_counter;
                                                    $new_result = \Illuminate\Support\Facades\DB::select("select target from `product_target_branch_totals` where `sap_code` = '". $record["ItemCode"] ."' AND `branch` = '". $old_dept_id[$dept] ."' AND `month` = '".$month."' AND `year` = '". intval($year_key) ."'");
                                                    $new_result = count($new_result) > 0 ? $new_result[0] : 0;
                                                    ?>
                                                <div class="text-sm">{{ $new_result ? ($new_result->target == 0 ? "" : $new_result->target) : ""  }}</div>
                                                    <?php array_push($new_tr, ($new_result ? $new_result->target : 0) ) ?>
                                    ...
                                    <th style="border: 2px solid black; z-index: 10; background-color: #fafad2;" class="border p-2">
                                        <div class="text-sm">{{ ($dept_sales*$record["ItemPrice"]) == 0 ? "" : number_format($dept_sales*$record["ItemPrice"]) }}</div>
                                    </th>
                                    <th style="border: 2px solid black; z-index: 10; background-color: #e4fdf7;" class="border p-2">
                                        <div class="text-sm">{{ ($dept_forecast*$record["ItemPrice"]) == 0 ? "" : number_format($dept_forecast*$record["ItemPrice"]) }}</div>
                                    </th>
                                    @php $a = ($dept_forecast*$record["ItemPrice"]) == 0 ? 0 : (((($dept_sales*$record["ItemPrice"])/($dept_forecast*$record["ItemPrice"]))*100)-100);  @endphp
                                    <th style="border: 2px solid black; z-index: 10; @if($a > 0) background-color: #cfffbd; @else background-color: #ffcbcb; @endif" class="border p-2">
                                        <div class="text-sm">{{ $a == 0 ? "" : number_format($a) }}</div>
                                    </th>
                                </tr>
                                @php
                                    ...
                                    $total_dept_s_value = $total_dept_s_value + ($dept_sales*$record["ItemPrice"]);
                                    $total_dept_f_value = $total_dept_f_value + ($dept_forecast*$record["ItemPrice"]);

                                    if (!array_key_exists($dept, $all_total_dept_sales)) {
                                        $all_total_dept_sales[$dept] = $dept_sales*$record["ItemPrice"];
                                    }
                                    else {
                                        $all_total_dept_sales[$dept] += $dept_sales*$record["ItemPrice"];
                                    }
                                    ...
                                    if (!array_key_exists($dept, $all_total_dept_tr)) {
                                        $all_total_dept_tr[$dept] = $dept_forecast*$record["ItemPrice"];
                                    }
                                    else {
                                        $all_total_dept_tr[$dept] += $dept_forecast*$record["ItemPrice"];
                                    }
